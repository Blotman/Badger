...
function XMLToTable( xmlString )
	...
	return collect(xmlString)
end

function LoadFromXMLFile( fileName )
	local xmlString = love.filesystem.read( fileName )
	local tableFromXML = XMLToTable( xmlString )
	local objectTable = tableFromXML[1]

	return InstantiateFromXMLTable( objectTable )
end

function InstantiateFromXMLTable( data, parent )
	local object = nil
	if data.label == "class" then
		object = Class.InstantiateFromXMLTable(data, parent)
	end
